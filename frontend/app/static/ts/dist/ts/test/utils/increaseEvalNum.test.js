...
const tmp = __importStar(require("../../src/increaseEvalNum"));
...
//     })
// })
(0, globals_1.describe)("test updateEval", () => {
...
function MockTranslation() {
    ...
    const displayTranslation = document.getElementById("display");
...
function setEvalNums(translationID, goodNum, badNum) {
 ...
 */
function testUpdateEval(before, after) {
    const translation = document.getElementById("translation0");
    const good = translation === null || translation === void 0 ? void 0 : translation.querySelector(".myapp-translation-goodBar");
    const bad = translation === null || translation === void 0 ? void 0 : translation.querySelector(".myapp-translation-badBar");
    (0, globals_1.test)(`good,badが押された回数がそれぞれ${before[0]},${before[1]}回のもと、${before[2]}ボタンが押されたとき`, () => {
        setEvalNums("translation0", parseInt(before[0], 10), parseInt(before[1], 10));
        tmp.updateEval("translation0", before[2]);
        // updateEval関数を実行した後の値と受け取った引数(after)の値を比較
        (0, globals_1.expect)(good.children[0].innerHTML).toBe(after[0]); // goodが押された回数
        (0, globals_1.expect)(bad.children[0].innerHTML).toBe(after[1]); // badが押された回数
        const goodStyle = window.getComputedStyle(good);
        const badStyle = window.getComputedStyle(bad);
        (0, globals_1.expect)(goodStyle.getPropertyValue("width")).toBe(`${after[2]}px`); // goodのwidth
        (0, globals_1.expect)(badStyle.getPropertyValue("width")).toBe(`${after[3]}px`); // badのwidth
    });
}
